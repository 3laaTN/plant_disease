...
#     def forward(self, x):
#         y = self.avg_pool(x)
#         y = self.conv(y.squeeze(-1).transpose(-1, -2))  # 使用Conv1d进行一维卷积
#         y = self.sigmoid(y)
...
#         # 假设BiFPN_Concat2和SimAM现在可以根据input_channels调整
...
#         self.cv1 = Conv(c1, 2 * self.c, 1, 1)
#         self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
#         # self.eca = ECA(k_size=eca_k_size)  # 创建ECA层
#         self.m = nn.ModuleList(GhostBottleneck(self.c, self.c, shortcut, g) for _ in range(n))
...
class GG2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = GSConv(c1, 2 * self.c,1,1)  # 使用GSConv替换Conv
        self.cv2 = GSConv((2 + n) * self.c, c2,1)  # 使用GSConv替换Conv
        self.m = nn.ModuleList(GhostBottleneck(self.c, self.c, shortcut, g) for _ in range(n))
        # self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
