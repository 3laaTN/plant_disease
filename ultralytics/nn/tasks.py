...
from ultralytics.nn.MSCA import MSCAAttention
from ultralytics.nn.MCA import MCALayer
from ultralytics.nn.ScConv import C2f_ScConv
from ultralytics.nn.ShuffleAttention import ShuffleAttention,C2f_MHSA,MHSA

# from ultralytics.nn.head.ASFF import ASFF2,ASFF3

import torch
import torch.nn as nn
# from ultralytics.nn.VanillaNet import VanillaBlock
from ultralytics.nn.modules import (GEC2f,BS,MC2f,MCALayer,AFPN,BiFPN_MCA,C2fSimAM,BE,EMA_attention,C2f_Faster_PConv,GC2f,GAMAttention,AIFI, C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    Classify, Concat, Conv, Conv2, ConvTranspose, Detect, DWConv, DWConvTranspose2d,
                                    Focus, GhostBottleneck, GhostConv, HGBlock, HGStem, Pose, RepC3, RepConv,
                                    RTDETRDecoder, Segment,CoordAtt,BiFPN_Concat2,BiFPN_Concat3,SimAM,GG2f,ECA,EC2f)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Segment,ASFF_Detect, Pose)):
            s = 256  # 2x min stride
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3,CoordAtt,"""VanillaBlock""",GG2f,ECA,EC2f,GAMAttention,GC2f,C2f_Faster_PConv,EMA_attention,C2fSimAM,C2f_ScConv,AFPN,MCALayer,MC2f,GEC2f,C2f_MHSA):
            c1, c2 = ch[f], args[0]
        ...
        # elif m in [BE]:
        #     BiFPN_MCA
        #     args = [ch[x] for x in f]  # Iterate over elements of f
           
            m_ = BE(dimension=1, eca_channels=c2, eca_k_size=3)
...
#    elif m in [BE]:
#             c2 = sum(ch[x] for x in f)
#             args = [ch[f], *args[1:]]  # Assuming the first argument is the number of channels
#             m_ = BE(dimension=1, bifpn_module=BE, eca_channels=ch[f], eca_k_size=3)  # Modify accordingly
